...
var GamesModelSchema = new Schema({
    name: String,
    image: String
});
var Game = mongoose.model('games', GamesModelSchema);
...
function isLoggedIn(req, res) {
    if (req.cookies.username) {
        console.log("===== isLoggedIn= ", req.cookies.username);
        return true;
    } else
        return false;
}
...
app.post('/login', function(req, res) {
    ...
    User.findOne({username:req.body.login_username, password:req.body.login_password_md5}, function(err, doc) {
        if (err) {
...
http.listen(3000, function() { console.log("Express server listening on port 3000"); });
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('create_room', function(data) {
        ...
        console.log("======= Crate room in progress ..");
        ...
        if(user_auth(data.r_creator) && game_type_exists(g_id, r_type)) { //checks if user is authorized and checks if game and game type exists
            ...
            io.emit('goto_room', { r_id: current, r_type: r_type});
        }
    });
